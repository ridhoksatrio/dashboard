...
try:
    ...
    try:
        ...
        rfm = pd.DataFrame() # Create an empty DataFrame to avoid errors
...
def get_strat(cid, data):
    """Menentukan strategi segmentasi berdasarkan rata-rata RFM dari Cluster ID."""
...
if not rfm.empty:
    for c in rfm['Cluster_KMeans'].unique():
        p = get_strat(c, rfm)
        profs[c] = p
        # Membuat label yang rapi (cth: 🏆 Champions (C1))
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div("🎯", className="met-icon"),
                html.Div(f"{rfm['Cluster_KMeans'].nunique() if not rfm.empty else 0}", className="met-val"),
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{'label':'🌐 All Segments','value':'all'}] + (
                            [{'label':f"{p['name']} - {champion_details[c]['tier']}" if p['name']=='🏆 Champions' and c in champion_details else p['name'], 'value':c} for c, p in profs.items()]
                        ),
                        value='all',
                        clearable=False,
...
def upd(sc, rr, sp):
    ...
    f1 = go.Figure(go.Pie(
        ...
        hole=.68,
        marker=dict(
            colors=[color_map.get(l, '#95A5A6') for l in cc.index],
    ...
    best_aov_segment = df.groupby('Cluster_Label')['AvgOrderValue'].mean().idxmax()
    best_aov_value = df.groupby('Cluster_Label')['AvgOrderValue'].mean().max()
    most_frequent_segment = df.groupby('Cluster_Label')['Frequency'].mean().idxmax()
    most_frequent_value = df.groupby('Cluster_Label')['Frequency'].mean().max()
