...
try: 
    ...
    try:
        ...
        # Fallback untuk Railway - buat data dummy jika file tidak ditemukan
        print("⚠️  File CSV tidak ditemukan, membuat data dummy untuk testing")
        np.random.seed(42)
        data = {
...
@server.route('/health')
def health_check():
    return {'status': 'healthy', 'message': 'Dashboard is running'}, 200

# Route utama untuk Dash
@server.route('/')
def dash_app():
    return app.index()
