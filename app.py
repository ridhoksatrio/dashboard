...
# ========== DEBUG INFO ==========
print("=" * 80)
print("🚀 CUSTOMER INTELLIGENCE DASHBOARD - FULL FEATURES")
print("=" * 80)
...
def load_data():
    ...
    try:
        # Cek file CSV
        csv_file = 'final_customer_segments.csv'
        ...
        if os.path.exists(csv_file):
            ...
            try:
                ...
                # Cari kolom yang sesuai dengan kebutuhan
                # Asumsi berdasarkan data Anda: kolom 2 = Frequency, kolom 3 = Monetary
                if len(df.columns) >= 4:
                    # Ambil kolom berdasarkan posisi (indeks 0-based)
                    freq_data = df.iloc[:, 2] if len(df.columns) > 2 else np.random.randint(1, 50, len(df))
                    money_data = df.iloc[:, 3] if len(df.columns) > 3 else np.random.randint(100, 10000, len(df))
                    
                    # Buat DataFrame dengan format yang diharapkan
                    rfm_data = {
                        'Recency': np.random.randint(1, 365, len(df)),
                        'Frequency': freq_data.values if hasattr(freq_data, 'values') else freq_data,
                        'Monetary': money_data.values if hasattr(money_data, 'values') else money_data,
                        'AvgOrderValue': np.random.randint(50, 500, len(df)),
                        'RFM_Score': np.random.randint(3, 15, len(df)),
                        'Cluster_KMeans': np.random.choice([0, 1, 2, 3, 4, 5, 6], len(df))
                    }
